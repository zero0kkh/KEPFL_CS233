...
class TestProject(unittest.TestCase):
    ...
    def test_1_folder_structure(self):
        ...
        for file in ["main2.py", "report.pdf"]:
            with self.subTest(f"Checking file {file}"):
                self.assertTrue(
                    ...
                    f"No file {file} found at {project_path}",
                )
        ...
        for file in ["__init__.py", "data.py", "utils.py"]:
            with self.subTest(f"Checking file src/{file}"):
                self.assertTrue(
                    (src_path / file).exists(), f"No file {file} found at {src_path}"
                )
        # Methods
        method_path = src_path / "methods"
        self.assertTrue(method_path.exists(), f"{method_path} not found")
        for file in ["__init__.py", "dummy_methods.py", "pca.py", "deep_network.py"]:
            with self.subTest(f"Checking file methods/{file}"):
                self.assertTrue(
                    (method_path / file).exists(),
                    f"No file {file} found at {method_path}",
                )
    ...
    def test_2_dummy_methods(self):
        ...
        _ = self._import_and_test("dummy_methods", "DummyClassifier", arg1=1)
    ...
    def test_4a_pca(self):
        ...
        self.assertIsInstance(
            exvar,
            float,
            f"pca.PCA.find_principal_components() should output a float, not {type(exvar)}",
        )
        self.assertIsInstance(
            pca.mean,
            np.ndarray,
            f"pca.PCA.mean should be an array, not {type(pca.mean)}",
        )
        self.assertIn(
            pca.mean.shape,
            ((D,), (1, D)),
            f"pca.PCA.mean has wrong shape ({pca.mean.shape} != {(D,)})",
        )
        self.assertIsInstance(
            pca.W, np.ndarray, f"pca.PCA.W should be an array, not {type(pca.W)}"
        )
        self.assertEqual(
            pca.W.shape,
            (D, d),
            f"pca.PCA.W has wrong shape ({pca.W.shape} != {(D, d)})",
        )
        with no_print():
        ...
        self.assertIsInstance(
            Y,
            np.ndarray,
            f"pca.PCA.reduce_dimension() should output an array, not {type(Y)}",
        )
        self.assertEqual(
            Y.shape,
            (N, d),
            f"pca.PCA.reduce_dimension() output has wrong shape ({Y.shape} != {(N, d)})",
        )
        ...
        proj = np.array(
            [-1.46, -0.55, 0.94, 0.35, 2.12, 1.50, -1.18, -0.75, -0.20, -0.76]
        ).reshape(N, d)
        with no_print():
            exvar = pca.find_principal_components(data)
            Y = pca.reduce_dimension(data)
        self.assertGreater(
            exvar,
            95.0,
            f"pca.PCA.find_principal_components() is not working on dummy data",
        )
        self.assertLess(
            np.linalg.norm(pca.mean - [1.41, 1.12]),
            0.01,
            f"pca.PCA.find_principal_components() is not working on dummy data",
        )
        self.assertLess(
            np.linalg.norm(pca.W - [[-0.89], [-0.45]]),
            0.01,
            f"pca.PCA.find_principal_components() is not working on dummy data",
        )
        self.assertLess(
            np.abs(Y - proj).max(),
            0.01,
            f"pca.PCA.reduce_dimension() is not working on dummy data",
        )
    ...
    def test_4b_deep_network(self):
        ...
        # Code structure
        trainer = self._import_and_test(
            "deep_network", "Trainer", nn.Linear(3, 3), lr, epochs, bs
        )
        ...
        for nn_type in ["MLP", "CNN"]:
            ...
            if nn_type == "MLP":
                train_dataset = TensorDataset(
                    torch.randn(N, D), torch.randint(0, C, (N,))
                )
                test_dataset = TensorDataset(torch.randn(N, D))
            elif nn_type == "CNN":
                train_dataset = TensorDataset(
                    torch.randn(N, 1, 32, 32), torch.randint(0, C, (N,))
                )
            ...
            self.assertIsInstance(
                preds,
                torch.Tensor,
                f"deep_network.{nn_type}.forward() should output a tensor, not {type(preds)}",
            )
            self.assertEqual(
                preds.shape,
                (bs, C),
                f"deep_network.{nn_type}.forward() output has wrong shape ({preds.shape} != {(bs, C)})",
            )
        # Test Trainer
        with no_print():
        ...
        self.assertIsInstance(
            pred_labels_test_torch,
            torch.Tensor,
            f"deep_network.Trainer.predict_torch() should output a tensor, not {type(pred_labels_test_torch)}",
        )
        self.assertEqual(
            pred_labels_test_torch.shape,
            (N,),
            f"deep_network.Trainer.predict_torch() output has wrong shape ({pred_labels_test_torch.shape} != {(N,)})",
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--no-hide", action="store_true", help="Enable printing from the student code"
    )
